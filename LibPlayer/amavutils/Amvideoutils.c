...
void amvideo_setscreenmode(float video_axis_ratio)
{
    ...
    if ((wh_ratio < (ratio4_3 + offset))
        ...
        memset(val, 0, sizeof(val));
        property_get("persist.sys.foregroundapp", val, "com.tcl.tv");
        if (strcmp(val, "com.tcl.videoplayer") == 0)
...
OUT:
    ...
    float axis_ratio;
    axis_ratio = (float)(dst_w /dst_h);

    amvideo_setscreenmode(axis_ratio);
