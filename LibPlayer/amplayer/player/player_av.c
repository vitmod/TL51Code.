...
int time_search(play_para_t *am_p, int flags)
{
    ...
    if (am_p->vstream_info.video_format == VFORMAT_H264MVC) {
        set_player_state(am_p, PLAYER_SEARCHOK);
        update_player_states(am_p, 1);
        set_player_error_no(am_p, PLAYER_UNSUPPORT_SEEK);
        return 0;
    }
    int sample_size;
    //-----------------------------------
...
void player_switch_sub(play_para_t *para)
{
    ...
    if (para->stream_type == STREAM_ES) {
		...
		if (am_getconfig_bool("sys.subtitlesavebuf.enable")) {
            property_set("sys.subtitlesavebuf.enable", 0);
            log_print("--switch sub ok, sys.subtitlesavebuf.enable 0\n");
        }
