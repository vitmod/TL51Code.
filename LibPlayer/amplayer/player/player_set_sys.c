...
static int cur_freq;
static int mali_l2_max_reads;
static int num_cores_enabled;
static int set_gpu_mali_flag;

void get_gpu_mali_info(void)
{
//echo 2 >  /sys/class/mpgpu/scale_mode
//echo 0 > /sys/class/mpgpu/cur_freq
//echo 1  >  /sys/module/mali/parameters/mali_l2_max_reads
//echo 1 > /sys/kernel/debug/mali/pp/num_cores_enabled
    scale_mode = get_sysfs_int("/sys/class/mpgpu/scale_mode");
    cur_freq = get_sysfs_int("/sys/class/mpgpu/cur_freq");
    mali_l2_max_reads = get_sysfs_int("/sys/module/mali/parameters/mali_l2_max_reads");
    num_cores_enabled = get_sysfs_int("/sys/kernel/debug/mali/pp/num_cores_enabled");
	//log_print("get_gpu_mali_info scale_mode:%d cur_freq:%d mali_l2_max_reads:%d num_cores_enabled:%d\n",
	//	scale_mode,cur_freq,mali_l2_max_reads,num_cores_enabled);
    return ;
}

void set_gpu_mali_for_maxbit(void)
{
	//log_print("set_gpu_mali_for_maxbit \n");
   //set_sysfs_int
    set_gpu_mali_flag = 1;
    set_sysfs_int("/sys/class/mpgpu/scale_mode",2);
    set_sysfs_int("/sys/class/mpgpu/cur_freq",0);
    set_sysfs_int("/sys/module/mali/parameters/mali_l2_max_reads",1);
    set_sysfs_int("/sys/kernel/debug/mali/pp/num_cores_enabled",1);
	#if 0
	int scale_mode_tmp = get_sysfs_int("/sys/class/mpgpu/scale_mode");
    int cur_freq_tmp = get_sysfs_int("/sys/class/mpgpu/cur_freq");
    int mali_l2_max_reads_tmp = get_sysfs_int("/sys/module/mali/parameters/mali_l2_max_reads");
    int num_cores_enabled_tmp = get_sysfs_int("/sys/kernel/debug/mali/pp/num_cores_enabled");
	log_print("get_set_gpu_mali scale_mode:%d cur_freq:%d mali_l2_max_reads:%d num_cores_enabled:%d\n",
		scale_mode_tmp,cur_freq_tmp,mali_l2_max_reads_tmp,num_cores_enabled_tmp);
	#endif
    return ;
}

void restore_gpu_mali(void)
{
	//log_print("restore_gpu_mali:%d \n",set_gpu_mali_flag);
    if (set_gpu_mali_flag == 1) 
    {
      // log_print("restore_gpu_mali scale_mode:%d cur_freq:%d mali_l2_max_reads:%d num_cores_enabled:%d\n",
	//	scale_mode,cur_freq,mali_l2_max_reads,num_cores_enabled);
        set_sysfs_int("/sys/class/mpgpu/scale_mode",scale_mode);
        set_sysfs_int("/sys/class/mpgpu/cur_freq",cur_freq);
        set_sysfs_int("/sys/module/mali/parameters/mali_l2_max_reads",mali_l2_max_reads);
        set_sysfs_int("/sys/kernel/debug/mali/pp/num_cores_enabled",num_cores_enabled);
        set_gpu_mali_flag = 0;
    }
	#if 0
	int scale_mode_tmp = get_sysfs_int("/sys/class/mpgpu/scale_mode");
...
void remove_gpu_mali_flag(void)
{
	//log_print("remove_gpu_mali_flag\n");
    set_gpu_mali_flag = 0;
    return;
}
