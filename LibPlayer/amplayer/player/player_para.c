...
static void get_stream_info(play_para_t *p_para)
{
    ...
    if (video_index != -1) {
        ...
        if (!unsupported_video && !am_getconfig_bool("media.amplayer.unsupport_video")) {
            int max_bit_rate = am_getconfig_int_def("media.amplayer.supportmaxbit", 160);
			//log_print("===================================================\n");
			log_print("max bit_rate :%d M\n",max_bit_rate);
			log_print("width:%d height:%d codec_id:%d bit_rate:%d frameRate:%d\n",p_para->vstream_info.video_width,
																						   p_para->vstream_info.video_height,
																						   p_para->vstream_info.video_format,
																						   p_para->pFormatCtx->bit_rate,
																						   p_para->vstream_info.video_rate);
			max_bit_rate = max_bit_rate*1024*1024;
		
			if (p_para->vstream_info.video_width * p_para->vstream_info.video_height > 2048 * 1088) {
					if (p_para->vstream_info.video_width*p_para->vstream_info.video_height > 4096*2304 ||
						   p_para->pFormatCtx->bit_rate > max_bit_rate) {
					     if (p_para->pFormatCtx->bit_rate > max_bit_rate) {
							// log_print("can't support bit_rate :%d M\n",p_para->pFormatCtx->bit_rate/1048576);
							 if (p_para->vstream_info.video_format == VFORMAT_H264 
							 	|| p_para->vstream_info.video_format == VFORMAT_H264_4K2K) {
							 	//set mali
							 	log_print("===set M===\n");
							 	get_gpu_mali_info();
								set_gpu_mali_for_maxbit();
							 }
					     } 
...
static int set_decode_para(play_para_t*am_p)
{
    ...
    */
    ...
    if (am_p->playctrl_info.no_video_flag) {
        ...
        if (am_p->file_type == RM_FILE && (am_p->astream_info.audio_format == AFORMAT_AC3)) {
