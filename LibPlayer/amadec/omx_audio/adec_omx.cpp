...
{
    m_codec=NULL;
    // here add code to wait mediaserver running
    unsigned char wait_time_out_count = 0; //  3 s
    sp<IServiceManager> sm = defaultServiceManager();
    sp<IBinder> binder = sm->getService(String16("media.player"));
    sp<IMediaPlayerService> service = interface_cast<IMediaPlayerService>(binder);
    while (service.get() == NULL && wait_time_out_count < 300 && !(*exit)) {
        usleep(10000);
        binder = sm->getService(String16("media.player"));
        service = interface_cast<IMediaPlayerService>(binder);
        wait_time_out_count--;
    }
    LOGE("wait mediaserver %d ms\n",wait_time_out_count*10);
    // code end
