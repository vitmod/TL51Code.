...
void *audio_decode_loop_omx(void *args)
{
    ...
    {
        pcm_with_raw = atoi(value);
    }
    adec_print("pcm_with_raw %d\n",pcm_with_raw);     
    while (1) {
exit_decode_loop:
        ...
        if (audec->StageFrightCodecEnableType == OMX_ENABLE_CODEC_DTSHD ||
            ...
            if (outlen > 8) {
                ...
                if (outlen + 8 <= outlen0) {
                    ...
                    if (outlen_raw > 6144*4) {
                        adec_print("err,dolby raw len error %d\n",outlen_raw);
                        outlen_raw = 0;
                    }
                }
                else {
                    adec_print("pcm size error,total len %d pcm len %d,\n", outlen0,outlen);
                }
            } else {
                outlen = 0;
            }
