...
#define AUDIO_FIRST_BITRATE (1024*15)
...
static AVIndexEntry *mov_find_next_sample(AVFormatContext *s, AVStream **st)
{
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        if(*st){
            ...
            BestMediaType = (*st)->codec->codec_type;
        }
        if (msc->pb && msc->current_sample < avst->nb_index_entries) {
            ...
            if (!sample || (!s->pb->seekable && current_sample->pos < sample->pos)){/*not seekable streaming,*/
               ...
               wantnew=1;
            }else if(s->pb->seekable && !s->pb->is_slowmedia){
                bit_rateKB = s->bit_rate/(1024 * 8);
                if ((bit_rateKB > AUDIO_FIRST_BITRATE))
                {
                    if (((avst->codec->codec_type != AVMEDIA_TYPE_AUDIO)
                        && (BestMediaType != AVMEDIA_TYPE_AUDIO))
                        ||((avst->codec->codec_type == AVMEDIA_TYPE_AUDIO)
                        &&(BestMediaType == AVMEDIA_TYPE_AUDIO))) {
                        if((FFABS(best_dts - dts) <= AV_TIME_BASE/2 && current_sample->pos < sample->pos) ||
                        ...
                        && (BestMediaType != AVMEDIA_TYPE_AUDIO)
                        && ((dts - best_dts) < AV_TIME_BASE)) {
                        wantnew=1;
                    }
                    else if ((avst->codec->codec_type != AVMEDIA_TYPE_AUDIO)
                        && (BestMediaType == AVMEDIA_TYPE_AUDIO)
                        && ((best_dts - dts) >= AV_TIME_BASE)) {
                        wantnew=1;
		    }
                }else if((FFABS(best_dts - dts) <= AV_TIME_BASE/2 && current_sample->pos < sample->pos) ||
                      (FFABS(best_dts - dts) > AV_TIME_BASE/2 && dts < best_dts)) {
                      wantnew=1;
                }
                //av_log(s, AV_LOG_ERROR, "dts=%llx,best=%llx,new=%d\n",dts,best_dts,wantnew);
