    ...
    if (find_next) {
        for(;;) {
            ...
            if ((pcr_pid <= 0 || (AV_RB16(buf + 1) & 0x1fff) == pcr_pid) &&
				...
				int64_t timestamp1 = mpegts_get_dts(s, stream_index, ppos, pos_limit);
                if((timestamp1 - timestamp > 0xfc20000)&&((timestamp1 - timestamp < 0xfe00000)
					||(timestamp1 == 0x8000000000000000))){ 
