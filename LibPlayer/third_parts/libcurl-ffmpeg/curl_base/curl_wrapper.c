...
{
    int def_size = CURL_FIFO_BUFFER_SIZE;
    int val;
    ...
    curl_h->curl = NULL;
    val = am_getconfig_int_def("libplayer.curl.buf_sizeK", 0);
    if (val > 0) {
        def_size = val * 1024;
    } else if (am_getconfig_bool_def("media.amplayer.low_ram",0)) {
        def_size = def_size / 4;
    }
    CLOGI("curl_wrapper_open with size %d KB\n", def_size/1024);
    curl_h->cfifo = curl_fifo_alloc(def_size);
    if (!curl_h->cfifo) {
