    ...
    do
    {
        ...
        for (;;)
        {
                     ...
                     start_time != AV_NOPTS_VALUE))
            {
                ...
                else if(st->first_dts  != AV_NOPTS_VALUE) duration -= st->first_dts;
...
static int av_estimate_timings(AVFormatContext *ic, int64_t old_offset)
{
    ...
    if(tmp) {
		 ...
		 // ellison {{
		 // fill in ic duration value
		 ic->duration = AV_TIME_BASE * (atoll(tmp + strlen("duration=")) / 1000);
		 av_log(NULL, NULL, "Skip calc duration, duration in url: %lld \n", ic->duration);
		 // ellison }}
...
#define DEFAULT_PROBE_SIZE 6500000
    ...
    if (fast_switch)
    {
				...
				&& strcmp(ic->iformat->name, "rtp"))
        {
            ...
            fast_switch = 1;/*enable fast switch for local playing.*/
        } else {
    ...
    for (i = 0; i < ic->nb_streams; i++)
    {
        ...
        {
            st->parser = av_parser_init(st->codec->codec_id);
			av_log(NULL, AV_LOG_INFO, "3880 st->need_parsing: %d\n", st->need_parsing);
			// ellison {{
			// force dolby parse headers
			if(CODEC_ID_AC3 == st->codec->codec_id ||
				CODEC_ID_EAC3 == st->codec->codec_id) {
				if(AVSTREAM_PARSE_NONE == st->need_parsing) {
					st->need_parsing = AVSTREAM_PARSE_HEADERS;
					av_log(NULL, AV_LOG_INFO, "force parsing audio headers for skEAC3\n");
				}
			}
			// ellison }}
