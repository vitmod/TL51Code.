...
static int matroska_resync(MatroskaDemuxContext *matroska, int64_t last_pos)
{
    ...
    while (!url_feof(pb)) {
        ...
        if(pb->pos > matroska->ctx->valid_offset && matroska->ctx->valid_offset > 0){
            av_log(matroska->ctx, AV_LOG_ERROR, "exceed valid offset\n");
            goto eof;
        }
                              ...
                              int64_t timestamp, int flags)
{
    ...
    if((index>0)&&(st->index_entries[index].timestamp >timestamp)&&(flags & AVSEEK_FLAG_ANY))
    {
        int64_t pre = st->index_entries[index - 1].timestamp;
        int64_t post = st->index_entries[index].timestamp;
        if((pre + post)/2 >timestamp)
            index -= 1;
    }
    index_min = index;
    for (i=0; i < matroska->tracks.nb_elem; i++) {
        ...
        //fix mkv  because of subtitle, for a while seek same postion.
         /* 
