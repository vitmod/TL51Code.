                                ...
                                AVFormatParameters *ap)
{
    ...
    if (pb) { /*fix inlegal address "buf, You can open the following comment out the code*/
        len = pb->buf_end - pb->buf_ptr;
        buf = pb->buffer;
        av_log(NULL, AV_LOG_INFO," adif parser header  len:%d\n",len);
    }
    /*  FFT: inlegal address "buf"  
    //add by xh,2010-08-24
    if (len&&buf[0]=='A' && buf[1]=='D' && buf[2]=='I' && buf[3]=='F')
    {
