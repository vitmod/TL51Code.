...
static int init_def_settings()
{
    ...
    av_log(NULL, AV_LOG_INFO, "get_data timeout error=%d get_data_timeout:%dS, , rtpqueue_cnt:%d\n\n",get_data_timeout_error, get_data_timeout, rtp_queue_cnt);
    return 0;
}
...
static int rtp_enqueue_packet(struct itemlist *itemlist, RTPPacket * lpkt){
    ...
    ITEM_LOCK(itemlist);
    if (itemlist->item_count >= rtp_queue_cnt && ltailpkt != NULL && (seq_greater_and_equal(ltailpkt->seq,lpkt->seq) == 1)) {
        ITEM_UNLOCK(itemlist);
        av_log(NULL, AV_LOG_INFO, "[%s:%d] cnt:%d,seq=%d Out of range,tailseq=%d\n", __FUNCTION__, __LINE__,itemlist->item_count,lpkt->seq,ltailpkt->seq);
...
static int rtp_read(URLContext *h, uint8_t *buf, int size)
{
	...
	if (s->use_cache) {
		...
		if (s->recvlist.item_count <= rtp_queue_cnt) {
			curtime = ff_network_gettime();
			if (starttime <= 0)
				starttime = curtime;
			if (gd_report_error_enable && (curtime > starttime + get_data_timeout*1000*1000) && !s->report_flag) {
