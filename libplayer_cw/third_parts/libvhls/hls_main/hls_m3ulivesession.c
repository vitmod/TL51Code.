...
//wujiaxi added for network connection status check
static int64_t last_net_disconnect_time = 0;
...
// }}
	...
	if (am_getconfig_int_def("net.ethwifi.up", 3) == 0) {
		if (last_net_disconnect_time == 0)
			last_net_disconnect_time = get_clock_monotonic_us();
	} else {
		if (last_net_disconnect_time != 0)
			net_disconnect_time = get_clock_monotonic_us() - last_net_disconnect_time;
		last_net_disconnect_time = 0;
	}
    ...
    if (last_fetch_timeUs + minPlaylistAgeUs <= nowUs) {
      ...
      //      LOGV("Reach to the time to refresh list");
       // }
		...
		//wujiaxi added for network connection status check
		if (net_disconnect_time > minPlaylistAgeUs)
			flag = 1;
    }
    return flag;
}
...
static int  _get_best_bandwidth_index(M3ULiveSession* s, SessionMediaItem * mediaItem) //rate adaptation logic
{
    ...
    if (s->bandwidth_item_num > 0 && adaptive_profile != 0 && s->fixbw == 0 && s->seek_step <= 0 && s->is_to_close <= 0) {
	   ...
	   if(bwquality == 0 || bwquality == 1) {
		   ...
		   //LOGV("s->bandwidth_item_num: %d, s->bandwidth_list: 0x%x\n", s->bandwidth_item_num, s->bandwidth_list);
...
// ellison added for playlist update
static int64_t sk_last_playlist_refesh_ts = 0;
...
rinse_repeat: {
		...
		// ellison {{
		// added for playlist update
		sk_last_playlist_refesh_ts = get_clock_monotonic_us();
		// ellison }}
        ...
        if (ret != 0) {
            ...
            if(!isLive && errcode > -599 && errcode < -299) {
                ...
                if(s->bandwidth_item_num > 1) {
                	// ellison {{
                	// try from top bw to bottom, don't switch automatically bw again
                    if(s->force_switch_bandwidth_index == -1 || s->force_switch_bandwidth_index == s->prev_bandwidth_index) {
                    	s->force_switch_bandwidth_index = (s->prev_bandwidth_index - 1 + s->bandwidth_item_num) % s->bandwidth_item_num;
                    }
                    // ellison }}
...
static int _download_next_segment(M3ULiveSession* s)
{
    ...
    if (s->playlist != NULL) {
        ...
        if (s->cur_seq_num < firstSeqNumberInPlaylist) {
            ...
            M3uBaseNode *tmp_node = NULL;
            int segment_switch = am_getconfig_int_def("libplayer.hls.segment_switch", 0);
            if(segment_switch == 0){
	...
	//wujiaxi fix bugs if server return a wrong stbid start at 20170104
    if (node == NULL) {
	...
	//wujiaxi fix bugs if server return a wrong stbid end
	...
	//LOGV("Wait to fetch segment\n");
	int waitNetwork = 0;
	int64_t playlist_ts_before_netdown = 0LL;
fetch_retry:
    while (am_getconfig_int_def("net.ethwifi.up", 3) == 0) {
		...
		// ellison {{
		// added for playlist update
        if(0 == playlist_ts_before_netdown) {
			playlist_ts_before_netdown = sk_last_playlist_refesh_ts;
        }
        waitNetwork = 1;
        // ellison }}
        _thread_wait_timeUs(s, NULL, 10 * 1000);
        
    }
	// ellison {{
	// check segment still in list
	if(isLive && waitNetwork){
		int wait10ms = 0;
		// check m3u8 refresh
		for(; wait10ms < 20; wait10ms ++) {
			if ((s->is_to_close >= 1) || (s->handling_seek > 0) || (s->seektimeUs >= 0) || (s->interrupt && s->interrupt())) {
	            return 0;
	        }
			if(playlist_ts_before_netdown == sk_last_playlist_refesh_ts){
				_thread_wait_timeUs(s, NULL, 10 * 1000);
			} else {
				break;
			}
		...
		int isSegmentStillInList = 1;
		pthread_mutex_lock(&s->session_lock);
		
		if (s->playlist != NULL) {
		
			int firstSeq = m3u_get_node_by_index(s->playlist, 0)->media_sequence;
			if(segment.media_sequence >= firstSeq && segment.media_sequence - firstSeq < m3u_get_node_num(s->playlist)) {
				isSegmentStillInList = 1;
			} else {
				isSegmentStillInList = 0;
				
			}
		} 
		pthread_mutex_unlock(&s->session_lock);
	
		if(!isSegmentStillInList) {
			LOGE("_fetch_segment_file. before download. segment not int list any more. %s\n", segment.fileUrl);
			fetch_retry_cnt = 0;
			goto skip_segment;
		}
		waitNetwork = 0;
		playlist_ts_before_netdown = 0;
	}
	// ellison }}
	...
	if ((ret == 0) && (s->seek_step > 0) && (segment.readOffset > 0 && segment.readOffset < segment.fileSize) && (interrupt_check(s) == 1)) {
        ...
        if(isLive){
			...
			if(!isSegmentStillInList) {
				LOGE("_fetch_segment_file. segment not int list any more. %s\n", segment.fileUrl);
				fetch_retry_cnt = 0;
			}
        }
        // ellison }}
...
skip_segment:
		...
		// ellison {{
		// return -1000 will case the worker thread exist, and the video play will block
        ret = HLSERROR(EAGAIN);
        // ellison }}
...
static void* _fetch_m3u8_worker(void* ctx)
{
	...
	last_net_disconnect_time = 0;
	//ellison {{
	// added for m3u8 status check
	sk_last_playlist_refesh_ts = 0;
	// ellison }}
    do {
        if (s->is_to_close > 0) {
            break;
...
#ifdef USE_SIMPLE_CACHE
    int cache_size_max = 1024 * 1024 * 15; //15M
    if (am_getconfig_bool_def("media.amplayer.low_ram", 0)) {
        cache_size_max = cache_size_max / 4;
    }
    ...
    if (session->is_mediagroup > 0) {
        ...
        if (session->is_livemode == 1) {
            ...
            // ellison {{
            	...
            	session->seek_step = 0;
            	session->seektimeUs = -1;
            	LOGE("m3u_session_seekUs posUs = 0, ignore it\n");
