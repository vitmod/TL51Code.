...
DDP_MediaSource::DDP_MediaSource(void *read_buffer)
{
    ...
    mStarted=false;
    mMeta=new MetaData;
    mMeta->setInt32(bExtendFormat, 2);
    mDataSource=NULL;
    mGroup=NULL;
    mBytesReaded=0;
    mCurrentTimeUs=0;
...
int DDP_MediaSource::get_frames_status(aml_audio_dec_t *audec){
    audec->dropped_nb_frames = dropped_nb_frames;
    audec->error_nb_frames = error_nb_frames;
    audec->decoded_nb_frames = decoded_nb_frames;
    return 0;
}
    ...
    *out = NULL;
	...
	if ((frame.len+read_size) > 6144) {
        ...
        if(head == 0x0b77 || head == 0x770b){
            ...
            if (head == 0x0b77 || head == 0x770b) {
                decoded_nb_frames++;
                dropped_nb_frames += readdiff/frame_size;
                error_nb_frames += readdiff/frame_size;
