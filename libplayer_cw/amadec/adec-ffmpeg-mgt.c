...
int find_audio_lib(aml_audio_dec_t *audec)
{
	...
	// ellison {{
	// skyworth ac3 soft decoder
	int is_passthrough = amsysfs_get_sysfs_int("/sys/class/audiodsp/digital_raw");
	if(!is_passthrough && 
		(ACODEC_FMT_AC3 == audec->format || 
		ACODEC_FMT_EAC3 == audec->format) &&
		(amsysfs_get_sysfs_int16("/sys/class/amaudio/dolby_enable") == 0)){
		adec_print("skEAC3 find_audio_lib. channels: %d, samplerate: %d, bitrate: %d, ext_data_size: %d, ext_data: 0x%02x %02x %02x %02x\n",
			audec->channels, audec->samplerate, audec->bitrate, audec->extradata_size, audec->extradata[0],audec->extradata[1],audec->extradata[2],audec->extradata[3]);
		{
		fd = dlopen("libamlskac3.so", RTLD_NOW);
	        if (fd != 0) {
				...
				if(adec_ops->init && 
					...
					adec_ops->release && 
					adec_ops->getinfo) {
					adec_print("found skac3 decoder lib\n");
					return 0;
				}
	        }
		}
	...
	// wujiaxi {{
	// skyworth dts soft decoder if not set passthouth
	if(!is_passthrough&&(ACODEC_FMT_DTS == audec->format) &&
		(amsysfs_get_sysfs_int16("/sys/class/amaudio/dts_enable") == 0)){
		fd = dlopen("libamlskdts.so", RTLD_NOW);
        if (fd != 0) {
            adec_ops->init    = dlsym(fd, "audio_dec_init");
            adec_ops->decode  = dlsym(fd, "audio_dec_decode");
            adec_ops->release = dlsym(fd, "audio_dec_release");
			...
			adec_print("found skdts decoder lib\n");
			return 0;
        }
	}
	// wujiaxi }}
