...
static int packet_queue_put_update(PacketQueue *q, AVPacket *pkt)
{
	...
	if ((q->nb_packets == q->max_packets) && (q->frames_for_seek_backward >= 1) ) {
			...
			log_print("%d reach maxpackets, mem:%d, firstpts:0x%llx, lastpts:0x%llx, diff_pts_ms:%lld",
				...
				(int64_t)((q->tail_valid_pts - q->head_valid_pts)*(q->timebase * 1000)));
		}
		...
		if (pkt->stream_index == s_avpkt_cache.video_index
			...
			if (((pkt->flags & AV_PKT_FLAG_KEY) == AV_PKT_FLAG_KEY || allow_no_key_flag == 1)
				...
				if (q->dur_calc_pts_start == -1) {
					...
					q->dur_calc_pts_end = pkt->pts;
					if ((q->dur_calc_pts_end - q->dur_calc_pts_start) >= (int64_t)(PTS_DURATION_CALC_TIME_MS/(q->timebase * 1000))) {
	...
	*/
	...
	if (pkt1->pts != AV_NOPTS_VALUE) {
		...
		*/
		if (abs(discontinue_pts) >= (int64_t)(PTS_DISCONTINUE_MS / (q->timebase * 1000))) {
	...
	*/
	...
	if (q->last_pkt->pts != AV_NOPTS_VALUE && pkt1->pts != AV_NOPTS_VALUE) {
...
static int avpkt_cache_update_player_param(av_packet_cache_t *cache_ptr)
{
    ...
    if(cache_ptr->has_audio){
		...
		if (player->astream_info.audio_duration > 0) {
			cache_ptr->queue_audio.timebase = (player->astream_info.audio_duration / PTS_90K);
		} else {
    ...
    if(cache_ptr->has_video){
		...
		if (player->vstream_info.video_pts > 0) {
			cache_ptr->queue_video.timebase = (player->vstream_info.video_pts / PTS_90K);
		} else {
	...
	if(is_bestv == 1) {
		...
		log_print("bestv online stream force keepframes 0\n");
	}
	
        if (!strncmp(player->file_name, "udp:", strlen("udp:"))) {
            cache_ptr->enable_keepframes = 0;
            log_print("udp force keepframes 0\n");
        }
	...
	if (cache_ptr->has_audio == 1) {
		...
		if (bitpktnum > 0) {
			...
			if (cache_ptr->bigpkt.data == NULL) {
				...
				cache_ptr->bigpkt.data = av_malloc(cache_ptr->bigpkt_size);
				if (cache_ptr->bigpkt.data != NULL) {
...
int avpkt_cache_put_update(av_packet_cache_t *cache_ptr, AVPacket *pkt) {
	...
	if(cache_ptr->has_audio && pkt->stream_index == cache_ptr->audio_index)
    {
		...
		cache_ptr->audio_cachems = (int64_t)(cache_ptr->queue_audio.bak_cache_pts * cache_ptr->queue_audio.timebase * 1000);
		...
		cache_ptr->video_cachems = (int64_t)(cache_ptr->queue_video.bak_cache_pts * cache_ptr->queue_video.timebase * 1000);
		...
		cache_ptr->sub_cachems = (int64_t)(cache_ptr->queue_sub.bak_cache_pts * cache_ptr->queue_sub.timebase * 1000);
...
static int avpkt_cache_queue_seek(PacketQueue *q, int64_t seekTimeMs)
{
	...
	seekPts = (int64_t)(seek_offset_ms / (q->timebase * 1000));
...
static int avpkt_cache_seek(av_packet_cache_t *cache_ptr, int64_t seekTimeMSec)
{
	...
	if (cache_ptr->has_audio) {
		...
		if (seek_suc == 1) {
			...
			if (ret == -1 || abs(diff_pts) > (int64_t)(PTS_DISCONTINUE_MS /(cache_ptr->queue_video.timebase * 1000))) {
				ret = avpkt_cache_queue_seek_bypts_frameid(&cache_ptr->queue_audio,
					cache_ptr->queue_video.cur_pkt->pts,
					cache_ptr->queue_video.cur_pkt->frame_id, 1);
			}
...
static int avpkt_cache_update_bufed_time(void) {
	...
	total_cache_ms = (int64_t)(
		s_avpkt_cache.queue_video.frames_in * frame_dur_pts * s_avpkt_cache.queue_video.timebase * 1000);
	...
	if (bak_calc_pts > 0) {
		cache_ms_bypts = (int64_t)(bak_calc_pts * s_avpkt_cache.queue_video.timebase * 1000);
...
static int64_t avpkt_cache_reduce_keepframes_ms(PacketQueue *q)
{
	...
	keep_ms = (int64_t)((s_avpkt_cache.keepframes * frame_dur_pts * q->timebase * 1000));
...
static int64_t packet_calc_cachetime_by_player_current_ms(PacketQueue *q, int64_t player_current_ms, int stream_idx, int debug)
{
	...
	q->frame_dur_pts = (int)(q->bak_cache_pts/q->frames_in);
...

	{
		...
		if (q->frame_dur_pts > 0) {
			...
			total_cache_ms = (int64_t)(q->frames_in * frame_dur_pts * q->timebase * 1000);
			...
			cache_ms_bypts = ((int64_t)(bak_calc_pts * q->timebase * 1000) - playtime_ms);
			if (debug) {
...
static int avpkt_cache_check_frames_reseved_enough(av_packet_cache_t *cache_ptr)
{
	...
	if (frame_dur_pts > 0) {
		...
		amstream_buf_ms = (int64_t)(frames_out * frame_dur_pts * pts_to_msec) - (current_ms - 
		...
		if (s_avpkt_cache.startenterkeepframes == 0
			&& amstream_buf_ms < cache_ptr->enterkeepframems) {
			return 1;
		} else {
			if (s_avpkt_cache.startenterkeepframes == 0) {
				s_avpkt_cache.startenterkeepframes = 1;
		...
		keepframe_ms = (int64_t)(cache_ptr->keepframes * frame_dur_pts * pts_to_msec);
		queue_left_ms =  (int64_t)((frames_in - frames_out) * frame_dur_pts * pts_to_msec);
...
int avpkt_cache_get(AVPacket *pkt)
{
	...
	while (1) {
		...
		if ((ret = avpkt_cache_check_can_get(&s_avpkt_cache,&stream_idx)) == 1) {
			if (s_avpkt_cache.enable_keepframes == 1 && s_avpkt_cache.queue_video.frames_in > 0) {
...
int avpkt_cache_task_close(play_para_t *player)
{
	...
	if (player->cache_thread_id != 0) {
		...
		avpkt_cache_interrupt_read(&s_avpkt_cache);
		while (retry < 200) {
