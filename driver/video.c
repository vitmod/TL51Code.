...
static struct statistics_delay_t delaystatistic;
...
static u32 vsync_fisr_remainder;
static u32 vsync_fisr_pts;
static u32 vsync_fisr_tolerance;
static u32 valid_vsync_count;
...
u32 get_frame_delay_count(void)
{
	int i = 0;
	u32 r_value = 0;
	u32 delay_count = 0;
	r_value = (valid_vsync_count >
		delaystatistic.out_fps) ? delaystatistic.bitindex : 0;
	bool last_frame_delay = false;
	for (i = r_value; i < delaystatistic.out_fps; i++) {
		u32 bit = i>>3;
		u32 left = i-(bit<<3);
		if (delaystatistic.value[bit] & (1<<left)
			&& last_frame_delay == false) {
			last_frame_delay = true;
			delay_count++;
		} else {
			last_frame_delay = false;
		}
...
static u32 delay_static_enable;
...
static void vsync_toggle_frame(struct vframe_s *vf)
{
	...
	if ((cur_dispbuf) && (cur_dispbuf != &vf_local) && (cur_dispbuf != vf)
		...
		if (delay_static_enable) {
			u32 valid_vsync_count_mod = valid_vsync_count%
				delaystatistic.out_fps;
			u32 div = div_u64_rem(90000ULL,
				delaystatistic.out_fps, &r);
			u32 bit = valid_vsync_count_mod>>3;
			u32 left = valid_vsync_count_mod-(bit<<3);
			delaystatistic.value[bit] &= (~(0x01<<left));
			delaystatistic.bitindex = valid_vsync_count_mod;
			delaystatistic.toggle[bit] |= (1<<left);
			delaystatistic.cur_frame_count = frame_count;
			u32 delay_mode = div > DUR2PTS(cur_dispbuf->duration) ?
				DUR2PTS(cur_dispbuf->duration) : div;
			if (vf->pts > timestamp_vpts_get()) {
				vsync_fisr_tolerance += div-
				(vf->pts-timestamp_vpts_get())%div;
				vsync_fisr_tolerance = vsync_fisr_tolerance%
				delay_mode;
				vsync_fisr_pts = vf->pts+vsync_fisr_tolerance;
				pr_info("large vsync =%d vf->pts=%d tolerance=%d duration=%d \n",
				vsync_fisr_pts,vf->pts,vsync_fisr_tolerance,DUR2PTS(cur_dispbuf->duration));
			} else if (vf->pts > 0) {
				vsync_fisr_tolerance -= div-
				(timestamp_vpts_get()-vf->pts)%div;
				vsync_fisr_tolerance = vsync_fisr_tolerance%
				delay_mode;
				vsync_fisr_pts = vf->pts+
				vsync_fisr_tolerance;
				pr_info("small vsync =%d vf->pts=%d tolerance=%d duration=%d \n",
				vsync_fisr_pts,vf->pts,vsync_fisr_tolerance,DUR2PTS(cur_dispbuf->duration));
			} else {
				vsync_fisr_tolerance += div-
				DUR2PTS(cur_dispbuf->duration)%div;
				vsync_fisr_tolerance = vsync_fisr_tolerance%
				delay_mode;
				vsync_fisr_pts = timestamp_vpts_get()+
				DUR2PTS(cur_dispbuf->duration)+
				vsync_fisr_tolerance;
				pr_info("null vsync =%d vf->pts=%d tolerance=%d duration=%d \n",
				vsync_fisr_pts,vf->pts,vsync_fisr_tolerance,DUR2PTS(cur_dispbuf->duration));
			}
		}
	 } else {
		...
		delaystatistic.out_fps = vinfo->sync_duration_num /
		...
		if (cur_dispbuf != vf) {
			if (delay_static_enable) {
				...
				delaystatistic.out_fps, &r);
				vsync_fisr_tolerance = div-
				DUR2PTS(vf->duration)%div;
				vsync_fisr_remainder = r;
				vsync_fisr_pts = vf->pts+vsync_fisr_tolerance;
			}
			...
			if (time_debug) {
				...
				if (first_frame_type) {
					pr_info("Stage 10 di->first_pic cost:%u\n",
						di_firstpic_time);
				} else if (get_first_quick_output())
					pr_info("Stage 11 di->first_pic cost:%u\n",
						di_firstpic_time);
				else
					pr_info("Stage 12 di->first_pic cost:%u\n",
						di_firstpic_time);
				...
				if (init_vformat == VFORMAT_H264 ||
					...
					get_vh264_firstcheckin_cmd1_time();
				} else if (init_vformat == VFORMAT_MPEG12) {
					set_cmd1_time =
						get_vmpeg12_set_cmd1_time();
					firstcheckin_cmd1_time =
					get_vmpeg12_firstcheckin_cmd1_time();
				} else if (init_vformat == VFORMAT_HEVC) {
					set_cmd1_time =
						get_vh265_set_cmd1_time();
					firstcheckin_cmd1_time =
					get_vh265_firstcheckin_cmd1_time();
				} else
					pr_info("UnSupport vformat debug:%d\n",
						init_vformat);
				cmd1_firstpic_time = first_picture_time -
					set_cmd1_time;
				pr_info("total keyframe->first pic cost:%u\n",
					cmd1_firstpic_time);
				open_time = get_am_open_time();
				init_firstpic_time = first_picture_time
					- open_time;
				pr_info("total start->first_pic cost:%u\n",
					init_firstpic_time);
				close_time =  get_am_close_time();
				lststop_firstpic_time = first_picture_time -
					close_time;
				pr_info("toal last_stop->first_pic cost:%u\n",
					lststop_firstpic_time);
				press_time = get_press_time();
				press_firstpic_time = first_picture_time -
					press_time;
				remove_waitkey_time = press_firstpic_time -
					firstcheckin_cmd1_time;
				pr_info("total(remove wait keyframe) cost:%u\n",
					remove_waitkey_time);
				pr_info("total switch channel cost:%u\n",
					press_firstpic_time);
			}
		}
		rpt_tab_idx = 0;
...
#endif
{
	...
	if (debug_flag & DEBUG_FLAG_VSYNC_DONONE)
		return IRQ_HANDLED;
	if (cur_dispbuf && first_frame_toggled
	&& timestamp_pcrscr_enable_state()) {
		u32 r;
		u32 div = div_u64_rem(90000ULL,
		delaystatistic.out_fps, &r);
		vsync_fisr_pts = vsync_fisr_pts+div;
		vsync_fisr_remainder += r;
		valid_vsync_count++;
		if (vsync_fisr_remainder >= delaystatistic.out_fps) {
			vsync_fisr_pts++;
			vsync_fisr_remainder -=
			delaystatistic.out_fps;
		}
	}
	u32 valid_vsync_count_mod = valid_vsync_count%
	...
	u32 bit = valid_vsync_count_mod>>3;
	u32 left = valid_vsync_count_mod-(bit<<3);
	delaystatistic.toggle[bit] &= (~(0x01<<left));
	delaystatistic.value[bit] &= (~(0x01<<left));
#ifdef CONFIG_SUPPORT_VIDEO_ON_VPP2
...
cur_dev->vpp_off,0,VPP_VD2_ALPHA_BIT,9);//vd2 alpha must set
	...
	if (delay_static_enable) {
		if (cur_dispbuf && first_frame_toggled) {
			if (valid_vsync_count_mod == delaystatistic.out_fps-1)
				frame_count_delay = get_frame_delay_count();
			if (vsync_fisr_pts > timestamp_vpts_get()+
			DUR2PTS(cur_dispbuf->duration)) {
				delaystatistic.value[bit] |= 1<<left;
				delaystatistic.bitindex = valid_vsync_count_mod;
				u32 diff=vsync_fisr_pts-timestamp_vpts_get()-
				DUR2PTS(cur_dispbuf->duration);
				if ( vsync_fisr_tolerance > diff )
					vsync_fisr_tolerance -= diff;
			} else {
				delaystatistic.bitindex = valid_vsync_count_mod;
			}
		}
...
static void video_vf_unreg_provider(void)
{
	...
	do
	{
		delaystatistic.value[delay_count] = 0;
		delaystatistic.toggle[delay_count] = 0;
		delaystatistic.out_fps = delaystatistic.out_fps-8;
		delay_count++;
	} while(delaystatistic.out_fps > 8) ;
	delaystatistic.bitindex=0;
	if (cur_dispbuf) {
...
static ssize_t video_delay_statistics_show(struct class *class,
			struct class_attribute *attr, char *buf)
{
	size_t len = 0;
	int i = 0;
	...
	for (i = r_value; i < delaystatistic.out_fps; i++) {
		...
		if (delaystatistic.value[bit] & (1<<left))
			len += sprintf(buf+len,
			"isr index %d =delay_yes\n", i-r_value);
		if (delaystatistic.toggle[bit] & (1<<left))
			frametoggle++;
	}
	for (i = 0; i < r_value; i++) {
		u32 bit = i>>3;
		u32 left = i-(bit<<3);
		if (delaystatistic.value[bit] & (1<<left)) {
			len += sprintf(buf+len, "isr index %d =delay_yes\n",
			delaystatistic.out_fps-r_value+i);
		}
		if (delaystatistic.toggle[bit] & (1<<left))
			frametoggle++;
	}
	len += sprintf(buf+len, "new_frame_count=%d\n",
		delaystatistic.cur_frame_count);
	len += sprintf(buf+len, "frametoggle=%d\n",
		frametoggle);
	return len;
}
...
static struct class_attribute amvideo_class_attrs[] = {
	...
	__ATTR_RO(video_delay_statistics),
	__ATTR_NULL
};
			 ...
			 void *para)
{
	...
	case VOUT_EVENT_MODE_CHANGE:
		...
		vsync_pts_inc = 90000 * vinfo->sync_duration_den /
				vinfo->sync_duration_num;
		vsync_pts_inc_scale = vinfo->sync_duration_den;
		memset(&delaystatistic, 0, sizeof(struct statistics_delay_t));
		delaystatistic.out_fps = vinfo->sync_duration_num/
				vinfo->sync_duration_den;
		vsync_pts_inc_scale_base = vinfo->sync_duration_num;
		spin_unlock_irqrestore(&lock, flags);
...
MODULE_PARM_DESC(delay_static_enable, "\n delay_static_enable");
module_param(delay_static_enable, uint, 0664);
...
MODULE_PARM_DESC(frame_count_delay, "\n frame_count_delay\n");
module_param(frame_count_delay, uint, 0664);
