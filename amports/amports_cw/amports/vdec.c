...
#ifdef VDEC_DEBUG_SUPPORT
			if (core->active_vdec) {
				if (core->active_vdec->start_run_clk == 0)
					pr_info("error, start_run_clk not set\n");
				/*pr_info("update run_clk %ld, %ld, %ld\n",
					get_current_clk(),
					core->active_vdec->start_run_clk,
					core->active_vdec->run_clk);*/
				core->active_vdec->run_clk +=
					(get_current_clk() -
					core->active_vdec->start_run_clk);
			}
...
#ifdef VDEC_DEBUG_SUPPORT
			vdec->run_count++;
			vdec->start_run_clk = get_current_clk();
			if (vdec->profile_start_clk == 0)
				vdec->profile_start_clk = get_current_clk();
			vdec->total_clk = get_current_clk()
				- vdec->profile_start_clk;
...
void vdec_fill_frame_info(struct vframe_qos_s *vframe_qos, int debug)
{
	...
	if (frame_qos_index >= QOS_FRAME_NUM) {
		memcpy(frame_info_buf_out, frame_info_buf_in,
			QOS_FRAME_NUM*sizeof(struct vframe_qos_s));
		memset(frame_info_buf_in, 0,
			QOS_FRAME_NUM*sizeof(struct vframe_qos_s));
		frame_qos_index = 0;
	...
	memcpy(&frame_info_buf_in[frame_qos_index++],
