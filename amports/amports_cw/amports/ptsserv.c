		...
		u32 *avg_bitare)
{
	...
	// ellison {{
	// fix can't enter buffing state
	if(timestampe_delayed < 0) timestampe_delayed = 0;
	pTable->last_pts_delay_ms = timestampe_delayed;
	// ellison }}
	...
	if (likely((pTable->status == PTS_RUNNING) ||
		...
		if (type == PTS_TYPE_VIDEO && pTable->first_checkin_pts == -1) {
			...
			if (time_debug) {
				...
				open_endtime = get_am_open_endtime();
				pr_info("first_checkin_vpts_time :%u\n",
					first_checkin_vpts_time);
				init_firstcheckin_time = first_checkin_vpts_time
					- open_endtime;
				pr_info("Stage 4 first vpts checkin cost:%u\n",
					init_firstcheckin_time);
			}
			/* } */
		}
