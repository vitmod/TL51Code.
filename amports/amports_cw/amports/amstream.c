...
static int amstream_port_init(struct port_priv_s *priv)
{
	...
	if (time_debug) {
		open_endtime = jiffies_to_msecs(jiffies);
		pr_info("amstream init end:%u\n", open_endtime);
		codec_init_time = open_endtime - open_time;
		pr_info("Stage 3 codec init cost:%u\n", codec_init_time);
	}
...
static int amstream_open(struct inode *inode, struct file *file)
{
	...
	if (time_debug) {
		open_time = jiffies_to_msecs(jiffies);
		pr_info("amstream open: %u\n", open_time);
		close_init_time = open_time - close_endtime;
		pr_info("Stage 2 stop->init cost:%u\n", close_init_time);
	}
...
static int amstream_release(struct inode *inode, struct file *file)
{
	...
	time_debug = get_time_debug();
	if (time_debug) {
		close_time = jiffies_to_msecs(jiffies);
		pr_info("amstream close: %u\n", close_time);
		press_time = get_press_time();
		press_close_time = close_time - press_time;
		pr_info("Stage 0 press->stop cost:%u\n", press_close_time);
	}
	...
	kfree(priv);
	if (time_debug) {
		close_endtime = jiffies_to_msecs(jiffies);
		pr_info("amstream close end:%u\n", close_endtime);
		codec_close_time = close_endtime - close_time;
		pr_info("Stage 1 codec close cost:%u\n", codec_close_time);
	}
 	...
 	case AMSTREAM_IOC_GET_AVINFO:
		{
			...
			if (this->type & PORT_TYPE_VIDEO) {
				...
				av_info.frame_format = get_frame_format();
				av_info.toggle_frame_count = get_toggle_frame_count();
				if (vdec_get_qos_info() != NULL && copy_to_user((void *)uarg->vframe_qos,
							vdec_get_qos_info(),
