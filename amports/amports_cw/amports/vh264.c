		...
		for (i = 0; (i < num_frame) && (!vh264_eos); i++) {
...
#ifndef ENABLE_SEI_ITU_T35
			{
			 ...
			 if (i) {
				...
				s_vframe_qos.max_mv = (reg_data >> 16) & 0xffff;
				s_vframe_qos.avg_mv = (reg_data >> 8) & 0xff;
				s_vframe_qos.min_mv =  reg_data & 0xff;
				reg_data = READ_VREG(AV_SCRATCH_L);
				s_vframe_qos.max_qp = (reg_data >> 16) & 0xff;
				s_vframe_qos.avg_qp = (reg_data >> 8) & 0xff;
				s_vframe_qos.min_qp =  reg_data & 0xff;
				reg_data = READ_VREG(AV_SCRATCH_M);
				s_vframe_qos.max_skip = (reg_data >> 16) & 0xff;
				s_vframe_qos.avg_skip = (reg_data >> 8) & 0xff;
				s_vframe_qos.min_skip =  reg_data & 0xff;
			 } else {
			 ...
			 if (decoder_debug_flag&0x2) {
				pr_info("max_mv %d    avg_mv %d  min_mv %d slice_type %d offset %x   i =  %d\n", s_vframe_qos.max_mv, s_vframe_qos.avg_mv, s_vframe_qos.min_mv, slice_type, b_offset, i);
				pr_info("max_qp %d    avg_qp %d  min_qp %d\n", s_vframe_qos.max_qp, s_vframe_qos.avg_qp, s_vframe_qos.min_qp);
				pr_info("max_skip %d  avg_skip %d  min_skip %d\n", s_vframe_qos.max_skip, s_vframe_qos.avg_skip, s_vframe_qos.min_skip);
			 }
			}
#endif
			frame_count++;
			...
			vdec_fill_frame_info(&s_vframe_qos, decoder_debug_flag&0x4);
...
static void vh264_put_timer_func(unsigned long arg)
{
	...
	if (atomic_read(&vh264_active))
		pc_active = READ_VREG(MPC_E);
	if (pc_active == 0) {
		pr_info("MPC_E reg is error\n");
		pc_count++;
		if (pc_count >= 50) {/*0.5s*/
			pr_info("------MPC_E REG is error, should force reset decoder!\n");
			pc_count = 0;
			schedule_work(&error_wd_work);
		}
	} else if (pc_active != 0 && pc_count > 0)
		pc_count = 0;
...
int vh264_dec_status(struct vdec_s *vdec, struct vdec_info *vstatus)
{
	...
	//vstatus->dec_frame_count = frame_count;
	//vstatus->dec_err_frame_count = error_frame_count;
	//vstatus->dec_drop_frame_count = decoded_drop_count;
	if (frame_count != gvs->frame_count) {
		//pr_info("%s,frame_count=%d,gvs->frame_count=%d\n", __FUNCTION__, frame_count, gvs->frame_count);
	}

        //if (error_frame_count != gvs->error_frame_count) {
        //        pr_info("%s,error_frame_count=%d,gvs->error_frame_count=%d\n",
	//		__FUNCTION__, error_frame_count, gvs->error_frame_count);
        //}

        if (decoded_drop_count != gvs->drop_frame_count) {
               // pr_info("%s,decoded_drop_count=%d,gvs->drop_frame_count=%d\n",
               //         __FUNCTION__, decoded_drop_count, gvs->drop_frame_count);
        }
