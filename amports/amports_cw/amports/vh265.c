...
static u32 dynamic_buf_num_margin_default = 9;
...
static void fill_frame_info(struct hevc_state_s *hevc,
	unsigned short slice_type, unsigned int framesize, unsigned int pts)
{
	unsigned char a[3];
	unsigned char i,j,t;
	u32 data;
...
//	pr_info(" mv data %x  a[0]= %x a[1]= %x a[2]= %x\n",data,a[0], a[1], a[2]);
	...
	vdec_fill_frame_info(vframe_qos, debug);
}
	...
	if (vf) {
		...
		if (pts_unstable && (hevc->frame_dur > 0))
			hevc->pts_mode = PTS_NONE_REF_USE_DURATION;
                ...
                fill_frame_info(hevc, slice_type, frame_size, vf->pts);
