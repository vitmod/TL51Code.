...
AmPlaylistFetcher::AmPlaylistFetcher(
    ...
    mBufferTimeUS = kMinBufferedDurationUs;
    ...
    if (property_get("media.hls.buffer_time_s", value, NULL) &&
        atoi(value) > 0) {
        mBufferTimeUS = atoi(value) * 1000000;
    } else {
        if (property_get("media.amplayer.low_ram", value, NULL) &&
            (!strcmp(value, "1") || !strcmp(value, "true"))) {
            mBufferTimeUS = 3000000; //low mem, to 3S
        }
    }
}
...
void AmPlaylistFetcher::onMonitorQueue() {
    ...
    int32_t targetDurationSecs;
    int64_t targetDurationUs = mBufferTimeUS;
    if (mPlaylist != NULL) {
        if (mPlaylist->meta() == NULL || !mPlaylist->meta()->findInt32(
    ...
    if (durationToBufferUs > mBufferTimeUS)  {
        durationToBufferUs = mBufferTimeUS;
    }
    ...
    if (mStreamTypeMask == AmLiveSession::STREAMTYPE_SUBTITLES) {
        ...
        for (size_t i = 0; i < mPacketSources.size(); ++i) {
            ...
            } else if (mPacketSources.keyAt(i) == AmLiveSession::STREAMTYPE_SUBTITLES)
                continue;
            if (bufferedDurationUs == 0 || bufferedStreamDurationUs < bufferedDurationUs) {
    ...
    if (finalResult == OK && (downloadMore || !mPostPrepared || mBuffering )) {
        ...
        postMonitorQueue((bufferedDurationUs / 2) > mBufferTimeUS
            ? (mBufferTimeUS / 2) : (bufferedDurationUs / 2), 1000000ll); // maybe pts wrong.
    }
}
