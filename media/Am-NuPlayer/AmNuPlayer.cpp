...
void AmNuPlayer::updateVideoSize(
    ...
    if (outputFormat != NULL) {
        ...
        if (property_get("media.omx.display_mode", value, NULL)) {
            no_need_notify = atoi(value);
        }
    } else {
	...
	char str_h[16] = {0};
	char str_w[16] = {0};
	sprintf(str_h,"%d",displayHeight);
	sprintf(str_w,"%d",displayWidth);
	property_set("media.libplayer.height",str_h);
	property_set("media.libplayer.width",str_w);
        ALOGV("updateVideoSize %d x %d", displayWidth, displayHeight);
        notifyListener(
...
status_t AmNuPlayer::getMediaInfo(Parcel* reply) {
    ...
    if (mSource->getDuration(&durationUs) == OK)
        reply->writeInt32(durationUs/1000000);
    else
        reply->writeInt32(-1);
	//filesize
    reply->writeString16(String16("-1"));
    ...
    int32_t bitrate;
    if (filemeta != NULL
            && filemeta->findInt32(kKeyBitRate, &bitrate) && bitrate > 0) {
    ...
    /*select info*/
    ssize_t selectedVideoTrack = mSource->getSelectedTrack(MEDIA_TRACK_TYPE_VIDEO);
    ssize_t selectedAudioTrack = mSource->getSelectedTrack(MEDIA_TRACK_TYPE_AUDIO);
	ssize_t selectedSubTrack = mSource->getSelectedTrack(MEDIA_TRACK_TYPE_SUBTITLE);
    reply->writeInt32(selectedVideoTrack);
    reply->writeInt32(selectedAudioTrack);
    reply->writeInt32(selectedSubTrack);

    
    /*build video info*/
    
	sp<MetaData> videometa = mSource->getFormatMeta(false);
	if (videometa != NULL)
	{  
	    reply->writeInt32(1);
        const char *_mime;
        ...
        int64_t duration;
	    reply->writeInt32(0);
	    if (videometa->findInt32(kKeyCodecID, &codecid))
            reply->writeInt32(codecid);
	    else
	   	    reply->writeInt32(-1);

        CHECK(videometa->findCString(kKeyMIMEType, &_mime));
	        reply->writeString16(String16(_mime));
	   
        if (videometa->findInt32(kKeyWidth, &width))
	   	    reply->writeInt32(width);
	    else
	   	    reply->writeInt32(-1);
	   
        if (videometa->findInt32(kKeyHeight, &height))
    ...
    /*build audio info*/
	sp<MetaData> audiometa = mSource->getFormatMeta(false);
	if (audiometa != NULL) {
		int32_t codecid, bitrate, samplerate, channelcount;
		int64_t duration;
        reply->writeInt32(1);
        reply->writeInt32(1);
	    if (audiometa->findInt32(kKeyCodecID, &codecid))
            reply->writeInt32(codecid);
		else
		    reply->writeInt32(-1);
		/*audio format*/
        ...
        if (audiometa->findInt32(kKeyChannelCount, &channelcount))
			reply->writeInt32(channelcount);
		else
			reply->writeInt32(-1);

		if (audiometa->findInt32(kKeySampleRate, &samplerate))
