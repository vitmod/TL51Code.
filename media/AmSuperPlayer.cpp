...
PASS_THROUGH:
        ...
        if (ret>0 && (match_codecs(type,value)|| match_codecs (type,"wmalossless") ||(!match_codecs(type,"ogg") && url_valid && IS_LOCAL_HTTP(muri)))) {
            /*some local http(127.0.0.1) dont support switch http clinet,use old AmlogicPlayer*/
            return AMLOGIC_PLAYER;
        }
        if ((match_codecs (type,"mp3") || match_codecs (type,"wma")) && url_valid && IS_HTTP(muri)) {
            return AMLOGIC_PLAYER;
        }
