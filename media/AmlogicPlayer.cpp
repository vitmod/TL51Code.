...
int AmlogicPlayer::NotifyHandle(int pid, int msg, unsigned long ext1, unsigned long ext2)
{
    ...
    case PLAYER_EVENTS_VIDEO_SIZE_CHANGED:
    {
        if(PropIsEnable("media.amplayer.vs.change.msg.on", 1) && !isSmoothStreaming){
            mWidth = ext1;
            mHeight = ext2;
	    if (video_rotation_degree == 1 || video_rotation_degree == 3) {// 90du,or 270du
                int32_t tmp = mWidth;
                mWidth = mHeight;
            	mHeight = tmp;
            }
                ...
                mHeight=mHeight*mAspect_ratio_den/mAspect_ratio_num;
	    {
            	char str_h[16] = {0};
    		char str_w[16] = {0};
            	sprintf(str_w,"%d",mWidth);
		sprintf(str_h,"%d",mHeight);
...
int AmlogicPlayer::UpdateProcess(int pid, player_info_t *info)
{
    ...
    if (info->status != PLAYER_ERROR && info->error_no != 0) {
        if (info->error_no == PLAYER_NO_VIDEO) {
	     ...
	     if(!mhasVideo)
	         sendEvent(MEDIA_PREPARED);				
        } else if (info->error_no == PLAYER_UNSUPPORT_SEEK) {
            sendEvent(MEDIA_INFO, MEDIA_INFO_NOT_SEEKABLE);
        }
        ...
        if (mHttpWV == false) {
            ...
            sendEvent(MEDIA_ERROR, MEDIA_ERROR_UNKNOWN, TAUDIO_VIDEO_NOT_SUPPORT);
            //sendEvent(MEDIA_ERROR,MEDIA_ERROR_UNKNOWN,info->error_no);
...
status_t AmlogicPlayer::start()
{
    ...
    if (sendUnsupportAgain == 1) {
        sendEvent(MEDIA_INFO, TAUDIO_NOT_SUPPORT);
        sendUnsupportAgain = 0;
    }else if (sendUnsupportAgain == 2) {
        sendEvent(MEDIA_INFO, TVIDEO_NOT_SUPPORT);
        sendUnsupportAgain = 0;
    }
...
status_t AmlogicPlayer::pause()
{
    ...
    if (mhasVideo || flag || mhasAudio) { /*video mode,and no video,no audio*/
        if (mDecryptHandle != NULL) {
            mDrmManagerClient->setPlaybackStatus(mDecryptHandle,
                                                 Playback::PAUSE, 0);
        }
    ...
    /*build audio info*/
        ...
        for (int i = 0; i < mStreamInfo.stream_info.total_audio_num; i ++) {        
            ...
            //reply->writeString16(String16(player_value2str("aformat", mStreamInfo.audio_info[i]->aformat)));
             if(mStreamInfo.audio_info[i]->aformat == AFORMAT_DTS){
               if(DtsHdStreamType==0x1){
                  mStreamInfo.audio_info[i]->aformat = AFORMAT_DTS_EXPRESS;
               }
               else if(DtsHdStreamType==0x2){
                  mStreamInfo.audio_info[i]->aformat = AFORMAT_DTS_HD_MASTER_AUDIO;
               }
            }
