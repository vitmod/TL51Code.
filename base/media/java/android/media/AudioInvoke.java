...
public class AudioInvoke {
    ...
    public static int getMaxVolume(Context context) {
        int mRet = 100;
        try {
            Class clzSoundManager = Class.forName("com.tcl.factory.FactoryManager");
            if(LOGI) Log.i(TAG,"Class SoundManager:" + clzSoundManager);
            ...
            Method mtdSoundManager$doGetAttribute = clzSoundManager
                    .getDeclaredMethod("doGetAttribute", new Class[] { int.class });
            if(LOGI) Log.i(TAG,"Method doGetAttribute:" + mtdSoundManager$doGetAttribute);
			
		if(1==(Integer)mtdSoundManager$doGetAttribute.invoke(objSoundManager,47)){
        	mRet = (Integer)mtdSoundManager$doGetAttribute.invoke(objSoundManager,55);
        	if(LOGI) Log.i(TAG,"FactoryManager doGetAttribute: Volume = " + mRet);
			}
        } catch (Exception e) {
            e.printStackTrace();
        }
