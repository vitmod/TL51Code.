...
public class MediaScanner
{
    ...
    //Added by TCL huyq mail:huyq@tcl.com for mediascan parameters define
    private static boolean mDisableScan = false;
	private static boolean mBulkInsertEnable = true;
    private static int mBulkInsertBufferSize = 50;
    private static int mBulkInsertAudioBufferSize = 5;
    private static int mScanFileCount =100000;
    private static int mScanCount =100000;
    // End TCL
	...
	//Added by TCL huyq mail:huyq@tcl.com  for set scan parameters
...
//End of TCL 
    ...
    private class MyMediaScannerClient implements MediaScannerClient {
        ...
        public void scanFile(String path, long lastModified, long fileSize,
            ...
            //Added by TCL huyq mail:huyq for limit max scan count &max scan file nums
            ...
            try {
                ...
                if (entry != null && (entry.mLastModifiedChanged || scanAlways)) {
                    if (noMedia) {
                        ...
                        //Changed by TCL huyq mail:huyq@tcl.com close processFile(audio & media)
                        if ((isaudio || isvideo) && scanAlways){
							Log.d(TAG,"scan audio or video file: " + path + scanAlways);
                            processFile(path, mimeType, this);
                        }

                        if (isimage && scanAlways) {
							Log.d(TAG,"scan image file: " + path + scanAlways);
...
//            Log.v(TAG, "scanFile: " + path + " took " + (t2-t1));
        ...
        private Uri endFile(FileEntry entry, boolean ringtones, boolean notifications,
			...
			//Changed by TCL mail:huyq@tcl.com for video insert database one by one
	...
	//Added by TCL huyq mail:huyq@tcl.com for cleanning database when plug out u disk
    public void updatescan(String updatepath, String volumeName)  {
        Log.w(TAG, "-----updatepath is :-------" + updatepath);
        ...
        if (updatepath != null) {
            where = MediaStore.Audio.Media.DATA + " like " + "'" + updatepath + "/%'";
        } else {
            where = null;
        }
        Log.w(TAG, "-----mAudioUri where is :-----" + where);

        // Delete the list of  Audio files from the content provider
        try{
            if (where != null) {
...
//Video
        ...
        if (updatepath != null) {
            where = MediaStore.Video.Media.DATA + " like " + "'" + updatepath + "/%'";
        } else {
            where = null;
        }
        Log.w(TAG, "-----mVideoUri where is :-----"+where);

        // Delete the list of Video files from the content provider
        try{
            if (where != null) {
                Log.w(TAG, "-----delete is :-----"+where);
...
//Images
        ...
        // Read existing files from the Image table
        if (updatepath != null) {
            where = MediaStore.Images.Media.DATA + " like " + "'" + updatepath + "/%'";
        } else {
            where = null;
        }
        Log.w(TAG, "-----mImagesUri where is :-----"+where);

        // Delete the list of Image files from the content provider
        try{
            if (where != null) {
                Log.w(TAG, "-----delete is :-----"+where);
...
//End of TCL
    ...
    public void scanDirectories(String[] directories, String volumeName) {
        try {
			...
			//Changed by TCL huyq mail:huyq@tcl.com for mediascan bulkinsert open/close
            if (mBulkInsertEnable) {
                // create MediaInserter for bulk inserts
                mMediaInserter = new MediaInserter(mMediaProvider, mBulkInsertBufferSize,mBulkInsertAudioBufferSize,mPackageName);
            }
			//End of TCL
            for (int i = 0; i < directories.length; i++) {
                // MStar Android Patch Begin
