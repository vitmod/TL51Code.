...
public class AudioService extends IAudioService.Stub {
    ...
    private static final int MSG_INVOKE_MIDWARE_AUDIO_SETVOLUME = 27; //caojun1@tcl.com asynchronous call AudioInvoke.setVolume
    ...
    //add by tank@tcl.com
    private static final int MSG_BROADCAST_AUDIO_TCL_VOLUME_INIT = 700;
    private static final int MSG_SCREENON_AUDIO_UNMUTE = 701;
	private static boolean tcl_volume_sync = false;//caojun1@tcl.com 
    //end tank@tcl.com
   ...
   /** @hide Maximum volume index values for audio streams */
    ...
    private final int[] STREAM_VOLUME_ALIAS_VOICE = new int[] {
        ...
        AudioSystem.STREAM_MUSIC,		//STREAM_USB_HEADSET
        AudioSystem.STREAM_MUSIC	//STREAM_USB_MIC
    };
    ...
    private static final int[] STEAM_VOLUME_OPS = new int[] {
        ...
        AppOpsManager.OP_AUDIO_MEDIA_VOLUME,    //STREAM_USB_HEADSET
        AppOpsManager.OP_AUDIO_MEDIA_VOLUME,    //STREAM_USB_MIC
    };
    ...
    public AudioService(Context context) {
       ...
       // Intialized volume
        ...
        if (mPlatformType == PLATFORM_TELEVISION) {
            DEFAULT_STREAM_VOLUME[AudioSystem.STREAM_MUSIC] = (MAX_MASTER_VOLUME * 3) / 10;
        }
		...
		tcl_volume_sync = false;//caojun1@tcl.com 
        //end tank@tcl.com
        // TODO merge orientation and rotation
    ...
    private void setSystemAudioVolume(int oldVolume, int newVolume, int maxVolume, int flags) {
        if (mSupportTvAudio) {
            setAudioMasterVolume((newVolume + 5) / 10);
            flags &= ~AudioManager.FLAG_SHOW_UI;
            sendMasterVolumeUpdate(flags, (oldVolume + 5) / 10, (newVolume + 5) / 10);
            return;
        }
        if (mHdmiManager == null
                || mHdmiTvClient == null
    ...
    public void adjustMasterVolume(int steps, int flags, String callingPackage, int uid) {
		if(!tcl_volume_sync){//caojun1@tcl.com [when the boot ok,exec tcl_volume_init only once ]
			 tcl_volume_init();
		 }
        if (mUseFixedVolume) {
            if (mSupportTvAudio) {
                ...
                setAudioMasterVolume(adjustVolume);
                flags &= ~AudioManager.FLAG_SHOW_UI;
                sendMasterVolumeUpdate(flags, curVolume, adjustVolume);
            }
            return;
        }
        ensureValidSteps(steps);
...
//Changed by TCL yangzp mail:yangzp for TCL TV volume policy impl 2014-12-10
...
//Changed by TCL yangzp mail:yangzp@tcl.com  2014-12-10
        // if (isMasterMute()) return 0;
//End of TCL
		if (mSupportTvAudio) {
            return getSaveAudioMasterVolume();
        }
    ...
    public void setMasterVolume(int volume, int flags, String callingPackage, int uid) {
		...
		int maxVolume = AudioInvoke.getMaxVolume(mContext);
        if(volume > maxVolume) {
            doSetMasterVolume((float)maxVolume / MAX_MASTER_VOLUME, flags);
            Log.d(TAG,"doSetMasterVolume: Now is the max volume "+maxVolume);
		return;
		}
    ...
    private void setAudioMasterVolume(int volume) {
        try {
            int curMasterVolume = getCurAudioMasterVolume();
            if (volume != curMasterVolume) {
...
//Added by TCL yangzp mail:yangzp for TCL TV volume policy impl 2014-12-10
	   ...
	   float tmp_oldvolume = (float)oldVolume / MAX_MASTER_VOLUME;
       int direction = (volume - tmp_oldvolume ) > 0 ? AudioManager.ADJUST_RAISE : AudioManager.ADJUST_LOWER;

       if (DEBUG_VOL)
           Log.d(TAG, "#########doSetMasterVolume oldVolume" + oldVolume + "direaction" + tmp_oldvolume + "newVolume = " + newVolume );

       if (isMasterMute() && (direction == AudioManager.ADJUST_RAISE)) {
           if (DEBUG_VOL)
                   Log.d(TAG, "#######doSetMasterVolume cancel mute mode ");
       ...
       sendMsg(mAudioHandler,MSG_INVOKE_MIDWARE_AUDIO_SETVOLUME,SENDMSG_REPLACE,newVolume,0,0,0);//caojun1@tcl.com asynchronous call AudioInvoke.setVolume
...
//End of TCL
    ...
    */
    ...
    private class AudioHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_INVOKE_MIDWARE_AUDIO_SETVOLUME:
                    AudioInvoke.setVolume(getMasterVolume(),mContext);
                    break;
                ...
                case MSG_PERSIST_MICROPHONE_MUTE:
                    ...
                    break;
                //add by tank@tcl.com
                case MSG_BROADCAST_AUDIO_TCL_VOLUME_INIT:
                    tcl_volume_init();
                    break;
                //end tank@tcl.com
		case MSG_SCREENON_AUDIO_UNMUTE:
			strResumeAudioUnMute();
			break;
            }
        }
    }
    ...
    //add by tank@tcl.com
    private void tcl_volume_init(){
		if(tcl_volume_sync){//caojun1@tcl.com 
			 return ;
		}
       int volume = AudioInvoke.getVolume(mContext);
	   boolean mute = AudioInvoke.isMute(mContext);//caojun1@tcl.com 
       ...
       Log.i(TAG,"tcl_volume_init Settings master volume:" + tmp + " ,eeprom master volume:" + volume);
       if(tmp != volume) {
	    ...
	    if(mute)//caojun1@tcl.com  [ if the midWare mute ==true,set mute ]
        {    
	           AudioInvoke.setMute(true, mContext);
        }
	   
	    tcl_volume_sync = true;//caojun1@tcl.com
    }
    //end tank@tcl.com
	private void strResumeAudioUnMute(){
		boolean state = false;
		if (isMasterMute()) {
			state = false;
			Slog.d(TAG,"## strResumeAudioUnMute");
			AudioInvoke.setMute(state, mContext);
			sendMsg(mAudioHandler, MSG_PERSIST_MASTER_VOLUME_MUTE, SENDMSG_REPLACE, state ? 1
				: 0, UserHandle.getCallingUserId(), null, PERSIST_DELAY);
		sendMasterMuteUpdate(state, 0);
		}
	}
    ...
    private void onSetA2dpSinkConnectionState(BluetoothDevice btDevice, int state)
    {
        ...
        {		
            SystemProperties.set("T968.bluth.state","off");	
            int curVolume = getMasterVolume();
            AudioSystem.setMasterVolume((float)curVolume/100);
			boolean mute = false;
            AudioSystem.setMasterMute(mute);		
        }else if (state == BluetoothProfile.STATE_CONNECTED){			
		    SystemProperties.set("T968.bluth.state","on");	
			int nMute = 0;
			nMute = SystemProperties.getInt("T968.speaker.mute" ,0);
            if (nMute > 0)
			{
			   boolean mute = true;
               AudioSystem.setMasterMute(mute);	
    ...
    private class AudioServiceBroadcastReceiver extends BroadcastReceiver {
        private class UsbAudioInfo {
            ...
            public UsbAudioInfo(Boolean playCapbility,Boolean captureCapbility){
                this.play = playCapbility ;
                this.capture = captureCapbility ;
            }
            public void setplayCapbility(Boolean hasPlayCapbility)
            {
                play = hasPlayCapbility;
            }
            public void setcaptureCapbilityr(Boolean hasCaptureCapbility)
            {
                capture = hasCaptureCapbility;
            }
            ...
            public String toString(){
                return "play:" + this.play + ",capture:" + this.capture ;
            }
         };

         Map<String,UsbAudioInfo> map = new HashMap<String,UsbAudioInfo>();
         private int RefCap = 0;
         private int RefPlay = 0;
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(Intent.ACTION_DOCK_EVENT)) {
               ...
               if (ID == null || ID.length() == 0) {
                  Log.e(TAG, "USB ID NULL pointer");
                  return;
              }

              if  ( state == 1 ) {
                  Boolean playback = false, capture = false;
                  if (ID.equals(usbHeadSetID)) {
                      if  (hasCapture) {
                              Log.d(TAG,"haier TV MIC capture found, alsaCard=" + alsaCard);
                              inDevice = AudioSystem.DEVICE_IN_USB_DEVICE;
                              if (0 == RefCap) {
                                  handleDeviceConnection((state==1) , inDevice, params);
                               }
                               capture = true;
                               RefCap++;
                          } else {
                               Log.d(TAG,"haier TV playback, alsaCard=" + alsaCard);
                               outDevice = AudioSystem.DEVICE_OUT_USB_DEVICE;
                               if  (0 == RefPlay) {
                                   handleDeviceConnection((state==1), outDevice, params);
                                }
                                playback = true;
                                RefPlay++;
                                map.put(ID, new UsbAudioInfo(playback,capture));
                          }
                              Log.i(TAG,"After usb plug RefPlay=" + RefPlay +", RefCap=" + RefCap);
                  }else {
                       if  (hasPlayback) {
                           Log.d(TAG,"state=1, playback=1, alsaCard=" + alsaCard);
                           outDevice = AudioSystem.DEVICE_OUT_USB_DEVICE;
                           if (0 == RefPlay) {
                                handleDeviceConnection((state==1), outDevice, params);
                            }
                          ...
                          if  (hasCapture) {
                              Log.d(TAG,"state=1, capture=1, alsaCard=" + alsaCard);
                              inDevice = AudioSystem.DEVICE_IN_USB_DEVICE;
                              capture = true;
                              if (0 == RefCap) {
                                  handleDeviceConnection((state==1) , inDevice, params);
              ...
              }else {  //usb audio plug out for usb headset not influence  other devices
                  for (Map.Entry<String,UsbAudioInfo> me:map.entrySet()) {
                      if  (me.getKey().equals(ID)) {
                          if  (me.getValue().getplayCapbility()) {
                              RefPlay--;
                              if (0 == RefPlay) {
                                  Log.d(TAG, "USB playback disconnected");
                                  outDevice = AudioSystem.DEVICE_OUT_USB_DEVICE;
                                  handleDeviceConnection((state==1) , outDevice, params);
                              }
                          }
                          if (me.getValue().getcaptureCapbility()) {
                              RefCap--;
                              if (0 == RefCap) {
                                  Log.d(TAG, "USB capture disconnected");
                                  inDevice = AudioSystem.DEVICE_IN_USB_DEVICE;
                                  handleDeviceConnection((state==1), inDevice, params);
                              }
                          }
                  ...
                  try {
                      ...
                      Log.v(TAG, "can not remove the map ID:" + ID);
                  }
                  Log.i(TAG,"After usb unplug RefPlay=" + RefPlay +", RefCap=" + RefCap);
                }
		...
		Slog.e(TAG," STR RESUME OR SCREEN ON ,will call unMute Audio state");
		sendMsg(mAudioHandler,MSG_SCREENON_AUDIO_UNMUTE,SENDMSG_REPLACE,0,0,null,0);
            ...
            //add by tank@tcl.com
                ...
                Slog.e(TAG,"TCL volume init recieve BOOT_COMPLETED ,will call tcl_volume_init");
                sendMsg(mAudioHandler,
                        MSG_BROADCAST_AUDIO_TCL_VOLUME_INIT,
                        SENDMSG_REPLACE,
                        0,
                        0,
                        null,
