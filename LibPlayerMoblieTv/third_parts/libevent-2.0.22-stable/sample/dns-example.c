...
gaic_getaddrinfo_cb(int result, struct evutil_addrinfo *res, void *arg)
{
	...
	if (result == EVUTIL_EAI_CANCEL) {
		//tt_assert(status->canceled);
            ...
            fprintf(stderr, "gaic_getaddrinfo_cb...\n");
...
gaic_cancel_request_cb(evutil_socket_t fd, short what, void *arg)
{
	...
	status->canceled = 1;
        fprintf(stderr, "gaic_cancel_request_cb...\n");
...
static void
gaic_launch(struct event_base *base, struct evdns_base *dns_base,char*ptr)
{
	...
	event_add(&status->cancel_event, &tv);
        fprintf(stderr, "gaic_launch ..\n");
	++pending;
}

void test_getaddrinfo_async_cancel(void *ptr)
{
	struct event_base *base;
...
main(int c, char **v) {
    ...
    char ip[128]={0};
    ret = getaddrinfo_async_cancel(v[idx],ip);
    fprintf(stderr, "resolving %s...,ret:%d\n",ip,ret);
