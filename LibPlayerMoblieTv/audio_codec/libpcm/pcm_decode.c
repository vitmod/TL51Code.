...
typedef struct {
    ...
    int mute_pcm_bluray_count;
    int pcm_bluray_need_mute_count;
} pcm_priv_data_t;
...
int audio_dec_decode(audio_decoder_operations_t *adec_ops, char *outbuf, int *outlen, char *inbuf, int inlen)
{
    ...
    *outlen = pcm_decode_frame(&pcm_read_ctl, outbuf, *outlen, audec);
    ...
    if (audec->format == AFORMAT_PCM_BLURAY) {
        if (pPcm_priv_data->pcm_bluray_need_mute_count== 0) {
            pPcm_priv_data->pcm_bluray_need_mute_count = pPcm_priv_data->pcm_samplerate *
            pPcm_priv_data->pcm_channels * mute_pcm_bluray_s;
        }
        if (pPcm_priv_data->mute_pcm_bluray_count < pPcm_priv_data->pcm_bluray_need_mute_count) {
            memset(outbuf, 0, *outlen);
            pPcm_priv_data->mute_pcm_bluray_count += *outlen;
        }
    }
