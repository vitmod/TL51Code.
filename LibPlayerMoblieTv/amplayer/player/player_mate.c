...
static int need_interrupt(play_para_t *player, player_cmd_t *cmd)
{
    ...
    // PLAYERING->FF&FB
    if(pctrl->hls_forward == 0 
        && pctrl->hls_backward == 0
        && pctrl->fast_forward == 0
        && pctrl->fast_backward== 0) {
    ...
    // FF->FB || FB->FF
    if((pctrl->hls_forward == 1 || pctrl->fast_forward) && cmd->ctrl_cmd == CMD_FB)
        return 1;
    if((pctrl->hls_backward == 1 || pctrl->fast_backward) && cmd->ctrl_cmd == CMD_FF)
        return 1;
    // FF&FB->PLAYING
    if(pctrl->hls_forward == 1 
        || pctrl->hls_backward == 1
        || pctrl->fast_forward == 1
        || pctrl->fast_backward == 1) {
        if(cmd->ctrl_cmd == CMD_FF || cmd->ctrl_cmd == CMD_FB) {
            if(cmd->param == 0)
                return 1;
        }
