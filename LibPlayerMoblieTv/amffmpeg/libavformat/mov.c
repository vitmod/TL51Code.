...
#define AVIO_SKIP_RETRY_TIMES (3)
static int mov_read_default(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    while (total_size + 8 < atom.size && !url_feof(pb) && !url_interrupt_cb()) {
        ...
        av_dlog(c->fc, "type: %08x '%.4s' parent:'%.4s' sz: %"PRId64" %"PRId64" %"PRId64",pos:%"PRId64"\n",
        a.type, (char*)&a.type, (char*)&atom.type, a.size, total_size, atom.size,avio_tell(pb));
        if (a.size == 0) {			
        ...
        if (!c->found_moov && (a.size > atom.size - total_size) && c->moov_retry) {
            if (!searchtag) {
                searchtag = serachenable;
                av_log(c->fc, AV_LOG_INFO, "******set search mode************\n");
            }
            av_log(c->fc, AV_LOG_WARNING, "'%.4s' atom size exceed totalsize, find next valid atom\n",(char*)&a.type);
            continue;
        }
        ...
        if (!parse) { /* skip leaf atoms data */
			if(!searchtag) {
				pb->seekflags |= URL_SEEK_FAIL_NO_BACK;
				while (skip_try <= AVIO_SKIP_RETRY_TIMES && !url_interrupt_cb()) {
					if (avio_skip(pb, a.size) >= 0) {
						skip_try = 0;
						break;
					}
					av_log(c->fc, AV_LOG_INFO, "L%d: skip_try:%d, skip_size:%lld\n", __LINE__,skip_try, a.size);
					skip_try++;
				}
				pb->seekflags &= ~URL_SEEK_FAIL_NO_BACK;
			}
            ...
            if (err < 0){
                av_log(c->fc, AV_LOG_ERROR,"type:'%.4s', parse error ret:%d\n",(char*)&a.type,err);
                return err;
            }
            ...
            if (left > 0){ /* skip garbage at atom end */
                pb->seekflags |= URL_SEEK_FAIL_NO_BACK;
                while (skip_try <= AVIO_SKIP_RETRY_TIMES && !url_interrupt_cb()) {
                    if (avio_skip(pb, left) >= 0) {
                        skip_try = 0;
                        break;
                    }
                    av_log(c->fc, AV_LOG_INFO, "L%d: skip_try:%d, skip:%lld\n", __LINE__,skip_try, left);
                    skip_try++;
                }
                pb->seekflags &= ~URL_SEEK_FAIL_NO_BACK;
            }
...
static int mov_read_stss(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    av_log(NULL,NULL,"%s,st->video_keyframe_count=%d\n",__func__,st->video_keyframe_count);
    for(i=0; i<entries; i++) {
