...
#define PLAYER_EVENTS_NEED_SEEK 22
...
static int rtsp_read_play(AVFormatContext *s)
{
    ...
    if (!(rt->server_type == RTSP_SERVER_REAL && rt->need_subscription)) {
        ...
        if (rt->state == RTSP_STATE_PAUSED) {
            ...
            if (rt->seek_timestamp == 0) {
                if(rt->headers && !strncmp(rt->headers, "Range: clock",12)){

                    av_log(NULL,NULL,"function:%s,line:%d,headers=%s\n",
                            __FUNCTION__,__LINE__,rt->headers);
                    av_log(NULL,NULL,"length=%d,sizof=%d\n", strlen(rt->headers),sizeof(cmd));
                    tmp = (char *)av_malloc(strlen(rt->headers) + 2);
                    strcpy(tmp, rt->headers);
                    av_strlcat(tmp, "-", strlen(rt->headers) + 2);
                    av_log(NULL,NULL,"tmp header:%s\n",tmp);
    ...
    while (rtspCtx->brunning) {
        ...
        if (ret < 0) {
            ...
            if(am_getconfig_int_def("net.ethwifi.up",3)==0) {
                 amthreadpool_thread_usleep(10);
                 rtspCtx->net_down = 1;
                 av_log(NULL,NULL,  "net down return\n");
                 continue;
            }
            if(ret == -1) {
                if(rtspCtx->net_down == 1) {
                    amthreadpool_thread_usleep(10000);
                    rtsp_read_close(rtspCtx->ctx);
                    av_log(NULL,NULL,"function:%s,line:%d,priv_data_size=%d\n",__FUNCTION__,
                            __LINE__,rtspCtx->ctx->iformat->priv_data_size);
                    if (rtspCtx->ctx->iformat->priv_data_size > 0) {
                        if (!(rtspCtx->ctx->priv_data =
                            av_mallocz(rtspCtx->ctx->iformat->priv_data_size))) {
                            ret = AVERROR(ENOMEM);
                            av_log(NULL, AV_LOG_ERROR, "[%s:%d\n]ENOMEM", __FUNCTION__, __LINE__);
                            break;
                        }
                        if (rtspCtx->ctx->iformat->priv_class) {
                            *(const AVClass**)rtspCtx->ctx->priv_data =
                                rtspCtx->ctx->iformat->priv_class;
                            av_opt_set_defaults(rtspCtx->ctx->priv_data);
                            if ((ret = av_opt_set_dict(rtspCtx->ctx->priv_data, &tmp)) < 0){
                                av_log(NULL, AV_LOG_ERROR, "[%s:%d\n]av_opt_set_dict failed",
                                    __FUNCTION__, __LINE__);
                                continue;
                            }
                        }
                    ...
                    if((ret = rtspCtx->ctx->iformat->read_header(rtspCtx->ctx, &ap)) < 0) {
                        av_log(NULL,NULL,"ret=read_header failed%d\n",ret);
                        break;
                    }
                    ffmpeg_id_notify(PLAYER_EVENTS_NEED_SEEK, 0, 0);
                    av_log(NULL,NULL,"debug send seek\n");
                    amthreadpool_thread_usleep(10000);
                    rtspCtx->net_down = 0;
                }
                continue;
...
static int rtsp_protocol_setcmd (URLContext *h, int cmd, int flag, unsigned long info, int64_t info1)
{
    RTSPProtocolContext *rtspCtx;
    RTSPState *rt;
