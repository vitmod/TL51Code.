...
static int dns_kick(URLContext *h, const char* hostname, int size, int port) {
    ...
    if (check_ip_string(hostname, size) == 1) {
...
redns:
    ...
    if (!hostname[0])
        ...
        if ((idx = dns_kick(h, hostname, strlen(hostname), port)) != -1) {
            if (idx == 0) {
    ...
    av_log(h, AV_LOG_INFO,"resolved %s:%d's ipaddress \n",hostname, port);
    cur_ai = ai;
    for (debug_cur_ai = ai; debug_cur_ai != NULL; debug_cur_ai = debug_cur_ai->ai_next) {
        addr = (struct sockaddr_in *)debug_cur_ai->ai_addr;
        av_log(h, AV_LOG_INFO,"current server ip:%s:%d\n", inet_ntop(AF_INET,
            &addr->sin_addr, ipbuf, 16), ntohs(addr->sin_port));
    }
 ...
 redo:
    ...
    if (ret < 0) {
        ...
        if (ret <= 0) {
			 av_log(h, AV_LOG_ERROR,
                   "TCP connection to %s:%d timeout failed!,cost time:%d\n",
                   hostname, port, timeout_ms);
            ret = AVERROR(ETIMEDOUT);
            goto fail;
        }
        /* test error */
    ...
    if (dnsfree == 1)
        freeaddrinfo(ai);
	av_log(h, AV_LOG_INFO,"tcp  connect %s used %d us\n",hostname,(int)(av_gettime()-tcp_starttime));	
    return 0;
